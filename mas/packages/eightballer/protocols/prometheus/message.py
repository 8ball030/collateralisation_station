...
_default_logger = logging.getLogger("aea.packages.eightballer.protocols.prometheus.message")
...
class PrometheusMessage(Message):
    ...
    protocol_id = PublicId.from_str("eightballer/prometheus:1.0.0")
    ...
    class _SlotsCls():
        __slots__ = (
            "callable",            "code",            "description",            "dialogue_reference",            "labels",            "message",            "message_id",            "performative",            "target",            "title",            "type",            "value",        )
    def __init__(
        self,
    ...
    def _is_consistent(self) -> bool:
        ...
        try:
            ...
            enforce(isinstance(self.performative, PrometheusMessage.Performative), "Invalid 'performative'. Expected either of '{}'. Found '{}'.".format(self.valid_performatives, self.performative))
            ...
            if self.performative == PrometheusMessage.Performative.ADD_METRIC:
                ...
                enforce(isinstance(self.type, str), "Invalid type for content 'type'. Expected 'str'. Found '{}'.".format(type(self.type)))
                enforce(isinstance(self.title, str), "Invalid type for content 'title'. Expected 'str'. Found '{}'.".format(type(self.title)))
                enforce(isinstance(self.description, str), "Invalid type for content 'description'. Expected 'str'. Found '{}'.".format(type(self.description)))
                enforce(isinstance(self.labels, dict), "Invalid type for content 'labels'. Expected 'dict'. Found '{}'.".format(type(self.labels)))
                for key_of_labels, value_of_labels in self.labels.items():
                    enforce(
                        isinstance(key_of_labels, str)
                    , "Invalid type for dictionary keys in content 'labels'. Expected 'str'. Found '{}'.".format(type(key_of_labels)))
                    enforce(
                        isinstance(value_of_labels, str)
                    , "Invalid type for dictionary values in content 'labels'. Expected 'str'. Found '{}'.".format(type(value_of_labels)))
            elif self.performative == PrometheusMessage.Performative.UPDATE_METRIC:
                expected_nb_of_contents = 4
                enforce(isinstance(self.title, str), "Invalid type for content 'title'. Expected 'str'. Found '{}'.".format(type(self.title)))
                enforce(isinstance(self.callable, str), "Invalid type for content 'callable'. Expected 'str'. Found '{}'.".format(type(self.callable)))
                enforce(isinstance(self.value, float), "Invalid type for content 'value'. Expected 'float'. Found '{}'.".format(type(self.value)))
                enforce(isinstance(self.labels, dict), "Invalid type for content 'labels'. Expected 'dict'. Found '{}'.".format(type(self.labels)))
                for key_of_labels, value_of_labels in self.labels.items():
                    enforce(
                        isinstance(key_of_labels, str)
                ...
                enforce(type(self.code) is int, "Invalid type for content 'code'. Expected 'int'. Found '{}'.".format(type(self.code)))
                if self.is_set("message"):
                    expected_nb_of_contents += 1
                    message = cast(str, self.message)
                    enforce(isinstance(message, str), "Invalid type for content 'message'. Expected 'str'. Found '{}'.".format(type(message)))
